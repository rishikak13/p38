...
function draw() {
  ...
  if (gameState===PLAY){
    ...
    if (camera.position.x+width/2  >ground.x + ground.width/2 ){
      ...
      invisibleGround = camera.position.x;
...
function spawnClouds() {
  ...
  if (frameCount % 60 === 0) {
    var cloud = createSprite(camera.position.x +50,height-300,40,10);
...
function spawnObstacles() {
  if(frameCount % 60 === 0) {
    var obstacle = createSprite(camera.position.x +50,height-95,20,30);
